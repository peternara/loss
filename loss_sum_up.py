# -*- coding: utf-8 -*-
"""
Created on Sat Feb  2 11:07:23 2019

@author: Administrator
"""
# referrnce :https://blog.csdn.net/qq_16949707/article/details/79929951
# sum up lossfor segmentation
...
epoch_loss += loss.data[0] print('{0:.4f} --- loss: {1:.6f}'.format(i * batch_size / N_train, loss.data[0])) 
optimizer.zero_grad() 
loss.backward() 
optimizer.step()
...
#use
 ...
 loss_val = loss.data[0] sum_epoch_loss += loss_val 
 if i == 0: 
     optimizer.zero_grad() 
     loss.backward()
#define
...
class WeightedBCELoss2d(nn.Module): 
    ...
    def forward(self, logits, labels, weights): 
        ...
        loss = loss * w loss = loss.sum() / w.sum() 
        return loss 
    ...
    def forward(self, logits, labels): 
        ...
        if self.is_weight: 
            ...
            if H == 128: 
                ...
                w0 = weights.sum() weights += ind * 2 
                w1 = weights.sum() weights = weights / w1 * w0 
...
def combined_loss(logits, labels, is_weight=True, is_log_dice=False): 
    size = logits.size() 
    assert size[1] == 1, size logits = logits.view(size[0], size[2], size[3]) 
    labels = labels.view(size[0], size[2], size[3]) 
    if is_weight: 
        batch_size, H, W = labels.size() 
        if H == 128: 
...
#------------------------------------------------------------dice loss--------------------------------
#reference:https://blog.csdn.net/xys430381_1/article/details/85883821
#reference:https://blog.csdn.net/a362682954/article/details/81226427
import torch
import torch.nn as nn

# define dice loss for single or multi class     
class DiceLoss(nn.Module):
    def __init__(self):
